 ...
 * LLM icin: OPENROUTER_API_KEY ortam degiskeni veya konsolda /key komutu
...
const VERI_KLASORU = path.join(__dirname, '..', 'hasinder-ai-data'); // araclar/ -> repo kökü
...
function eslesmeDetay(kullaniciKelimeler, soruKelimeler) {
  ...
  return {
    skor: eslesme / Math.sqrt(kullaniciKelimeler.length * soruKelimeler.length),
    sayi: eslesme
  };
}

// Kalitesiz sinav kayitlari: cevabi bos veya "cevap yok" olanlar elenir
function gecerliCevapMi(o) {
  const c = String(o.cevap || '').toLocaleLowerCase('tr');
  if (c.trim().length < 15) return false;
  if (c.includes('mevcut değil') || c.includes('cevap yok')) return false;
  return true;
}
...
function veriYukle() {
  ...
  // Sinav dosyalarindaki secenek parcaciklari (dogruMu alanli) ASLA cevap olarak
  // donulmez; sadece gercek soru-cevap kayitlari ana havuzda kullanilir.
  const qa = tumQa.filter(o => !('dogruMu' in o) && gecerliCevapMi(o));
...
function enIyiEslesme(veri, girdi) {
  ...
  for (const k of veri.qa) {
    const d = eslesmeDetay(kelimeler, k.kelimeler);
...
function terimBul(veri, girdi) {
  ...
  for (const t of veri.terimler) {
    ...
    const ilkKelimeUygun = tk[0] && kelimeler.includes(tk[0]) && tk[0].length >= 3;
...
/* ---------- OpenRouter LLM API ---------- */
// Kullanici sorusuyla en iyi eslesme yapan ilk N kaydi bul (LLM prompt'u icin)
function enIyiEslesmeler(veri, girdi, adet = 5) {
  const kelimeler = icerikKelimeleri(girdi);
  const skorlu = [];
  for (const k of veri.qa) {
    const d = eslesmeDetay(kelimeler, k.kelimeler);
...
// Sadece kullanici sorusuyla en iyi eslesme yapan 5 kaydi prompt'a ekle
function bilgiBankasiMetni(veri, girdi) {
  const enIyi = enIyiEslesmeler(veri, girdi, 5);
  const qc = enIyi.map(o => `S: ${o.soru}\nC: ${o.cevap}`).join('\n\n');
...
async function llmSor(veri, gecmis, apiKey, model, soru) {
  ...
  const yanit = await fetch(OPENROUTER_URL, {
    method: 'POST',
    headers: { 'Authorization': 'Bearer ' + apiKey, 'Content-Type': 'application/json' },
    body: JSON.stringify({ model, messages: [{ role: 'system', content: sistem }, ...gecmis.slice(-10)], temperature: 0.7, max_tokens: 2000, reasoning: { exclude: true } })
  });

  if (!yanit.ok) {
    gecmis.pop();
    const hata = await yanit.text();
    throw new Error(`OpenRouter API hatasi (${yanit.status}): ${hata.slice(0, 200)}`);
  }
...
async function cevapUret(veri, gecmis, apiKey, model, girdi) {
  ...
  if (apiKey) {
    try {
      return { metin: await llmSor(veri, gecmis, apiKey, model, girdi), kaynak: 'Bulut LLM (OpenRouter)' };
    } catch (e) {
...
const YARDIM = `Komutlar:
  /key <anahtar>   OpenRouter API anahtarini ayarla (yedek bulut LLM icin)
  /model <adi>     Model degistir (varsayilan: nvidia/nemotron-3-super-120b-a12b:free)
...
async function main() {
  ...
  // rl.on('line') ile satirlari dinle (async cevaplar icin guvenli)
  rl.on('line', async (satir) => {
    const girdi = satir.trim();
    if (!girdi) { rl.prompt(); return; }
    const norm = normalize(girdi);
    ...
    if (['cikis', 'exit', 'quit'].includes(norm)) {
      ...
      else console.log('Kullanim: /key sk-or-v1-xxxxxx');
